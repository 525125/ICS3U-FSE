...
class character:
    ...
    def check_ground(self): # check if player is on the ground or a platform
        self.fallRect = Rect(self.x - self.w // 2, self.y - self.h // 2 + 2, self.w, self.h)
    ...
    def animate(self): # animates all character actions
        ...
        if self.invFrames > 0: # player flashes when taking damage
            ...
            self.cover.set_alpha((5 - (self.invFrames % 5)) * 50)
    ...
    def shoot(self): # player shooting
        if self.warmUp == 0 and not self.lazer:
            ...
            if self.direction[-1] == 'R':
                lazers.append([self.x + self.w // 2 - 13, self.y - 4, False, self.direction[-1]])
            else:
                lazers.append([self.x - self.w // 2 - 30, self.y - 4, False, self.direction[-1]])
    ...
    def show_death(self): # show the player's death and game over
        ...
        for i in range(len(playerPics[self.direction])):
            ...
            self.w, self.h = self.pic.get_size()
            grey = 255-i*255/len(playerPics[self.direction])
...
class movePlatform: # vertically and horizontally moving platforms
    ...
    def moveH(self): # move horizontally
        ...
        # move player if player on platform; check if boundary is reached
        if self.direction == 'right':
            self.x += 2
...
class finalBoss:
    ...
    def move(self): # chooses or executes an action
...
def updateMets(mets, projectiles, lazers, offset, screen): # animate mets that are on the screen and check for damage
    # this is almost exactly the same as the 'updateArmadillos' function
    global score
    remove = []
    dead = []
...
def fadeIn(screen, fps):
    screencopy = screen.copy()
    fadeSurface = Surface((800, 600)).convert()
    for i in range(int(fps*2)):
        fadeSurface.set_alpha(255 - i/(fps*2)*255)
        screen.blit(screencopy, (0, 0))
        screen.blit(fadeSurface, (0, 0))
        display.flip()
    screen.blit(screencopy, (0, 0))

def fadeOut(screen, fps):
    screencopy = screen.copy()
    fadeSurface = Surface((800, 600)).convert()
    for i in range(int(fps*2)):
        fadeSurface.set_alpha(i/(fps*2)*255)
        screen.blit(screencopy, (0, 0))
        screen.blit(fadeSurface, (0, 0))
        display.flip()

def backgroundMusic(type): # type is the music to be played
    if type == 'menu':
...
load_text = bitfont40.render('LOAD', 0, (255, 255, 255, 255))
...
progress = 1 # holds furthest level player has reached
